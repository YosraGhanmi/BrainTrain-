...
export default async function TeacherVerifyPage({
  ...
  const t = await getTranslations({ locale: params.locale, namespace: 'teacherPortal.verify' });
  ...
  return (
    <BrandedAuthPanel
      ...
      heading={[t('oneMore'), t('stepToGo')]}
      tagline={t('tagline')}
    >
      <p className="mt-6 text-sm text-stone">{t('body')}</p>

      <form action={verifyTeacherSecretCode} className="mt-8 space-y-7">
        <input type="hidden" name="locale" value={params.locale} />

        <div className="space-y-2">
          <label className="text-sm font-semibold uppercase tracking-[0.2em] text-stone">{t('secretCode')}</label>
          <input
            name="code"
            type="text"
            inputMode="numeric"
