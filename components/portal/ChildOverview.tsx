        ...
        <div className="mt-4 space-y-4">
          {child.enrollments.map((enrollment) => {
            ...
            return (
              <Link
                key={enrollment.id}
                href={`/parent-portal/children/${child.id}/courses/${enrollment.id}`}
                className="block rounded-2xl border border-ink/10 bg-white p-6 shadow-soft transition hover:border-accent/40"
              >
                ...
                {enrollment.notes.length > 0 ? (
                  <p className="mt-5 flex items-center gap-2 border-t border-ink/10 pt-4 text-xs font-semibold text-stone">
                    <MessageSquare className="h-3.5 w-3.5" />
                    {enrollment.notes.length} teacher note{enrollment.notes.length === 1 ? '' : 's'} · view details
                  </p>
                ) : null}
              </Link>
            );
