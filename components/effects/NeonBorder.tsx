...
function buildArc(lap: number, lengthPct: number, w: number, h: number, color: string) {
  const fw = w > 0 ? w : 100;
  const fh = h > 0 ? h : 100;
...
export default function NeonBorder(props: Props) {
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(() => {
      ...
      if (r.width === sizeRef.current.w && r.height === sizeRef.current.h) return;
  ...
  useEffect(() => {
    ...
    const frame = (now: number) => {
      ...
      if (s > 0) {
        ...
        if (a) {
          a.style.setProperty('--arc', buildArc(lap, p.borderSize, w, h, p.color));
        }
